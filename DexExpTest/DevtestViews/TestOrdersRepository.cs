...
namespace DexExpTest
{
	...
	public class TestOrdersRepository : OrderRepository
	{
		...
		{
			Order order = new Order(new DateTime(2014, 1, 1).AddDays(random.Next(0, 60)), number % 3 == 0, RandomItem<Product>(products), random.Next(1, 100));
			return order;
		}
